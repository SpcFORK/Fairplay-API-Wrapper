...
const browser = !!globalThis.window,
  // @ DEPS
  axios = !browser ? require("axios") : false,
  cheerio = !browser ? require("cheerio") : false,
  // @ DATA
  ENDURLS = {
  ...
  ENDPOINTS = {
    ...
    front_resources: {
      ...
      isFP_CI(core_inst) {
        switch (core_inst) {
          case typeof FP_Shard:
            return true;
          case typeof FP_Load_Balancer:
            return true;
          case typeof FP_Batch_Processing_Server:
            return true;
          default:
            return false;
        }
  ...
  try {
    if (!browser) {
      ...
      if (res.headers.get("content-type") === "application/json") {
        ...
        let domparsed = new DOMParser().parseFromString(text, "text/html");
...
function handle_machinesArr(machinesArr = []) {
  ...
  for (let i = 0; i < machinesArr.length; i++) {
    ...
    else if (contains(n_, "batch processing server"))
      fn_ = "batchprocessingserver";
    else {
      console.log(machinesArr[i]);
      throw new Error(
        "Unseen/New Machine Type, Contact Developer, may be error.",
      );
    }
...
async function get_front_resources() {
  // Front has been depricated, use back instead.
  let res = await get_resources(ENDPOINTS.back_resources);
...
class RequestQueue {
  ...
  tryToProcessNextQuery() {
    if (!this.ready)
      setTimeout(() => this.tryToProcessNextQuery(), this.cooldown);
...
// @ Runtime Check
!browser ? (module.exports = __EXP__) : (globalThis.fpAPI = __EXP__);
